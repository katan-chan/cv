...
\subsection{Normalized Landmark Space}
...
Để phục vụ cho các thuật toán xử lý hình học,
...
\subsection{Cấu trúc hình học của bàn tay}

Cấu trúc xương của bàn tay nhận diện từ ảnh được biểu diễn dưới dạng một khung xương gồm 21 điểm. Cấu trúc này bắt đầu từ điểm gốc tại cổ tay (wrist) và phân nhánh thành năm ngón tay. Mỗi ngón tay được biểu diễn bởi bốn điểm khớp nối tiếp: khớp đốt bàn ngón (MCP), khớp liên đốt gần (PIP), khớp liên đốt xa (DIP) và đầu ngón tay (TIP). Cách biểu diễn này giúp nắm bắt đầy đủ các bậc tự do chuyển động, cho phép suy luận chính xác về tư thế và cử chỉ của bàn tay từ dữ liệu hình ảnh (xem Hình \ref{fig:hand_landmark}).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/HandLandmarks.png}
    \caption{Sơ đồ cấu trúc các điểm đặc trưng trên bàn tay.}
...
phát hiện và trích xuất các điểm đặc trưng của bàn tay
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/mediapipe_hand_landmarks.png}
    \caption{Ví dụ phát hiện landmarks bàn tay sử dụng MediaPipe.}
    \label{fig:mediapipe_hand}
\end{figure}

MediaPipe đạt tốc độ xử lý cao
và hoạt động ổn định trên CPU, 
